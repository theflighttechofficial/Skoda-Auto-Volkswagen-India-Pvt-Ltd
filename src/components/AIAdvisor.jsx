...
export const AIAdvisor = ({ brand = "skoda", initialPrompt }) => {
  ...
  const primaryColor = isVW ? "blue" : "emerald";
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
            <Bot
              className={`w-6 h-6 ${isVW ? "text-blue-400" : "text-emerald-400"}`}
            />
            {isVW
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-400">
          <span
            className={`w-2 h-2 rounded-full ${isVW ? "bg-blue-400" : "bg-emerald-400"} animate-pulse`}
          />
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {modelOptions.map((m) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md transition-all font-medium ${selectedModel === m ? (m.includes("vRS") || m.includes("GTI") || m.includes("GT") ? "bg-red-600 text-white shadow-sm font-bold" : isVW ? "bg-blue-600 text-white shadow-sm" : "bg-emerald-600 text-white shadow-sm") : "bg-zinc-950 text-zinc-400 hover:text-white border border-zinc-800"}`}
            >
              {m}
            </button>
          ))}
        ...
        <div className="flex items-center gap-1.5 ml-0 sm:ml-4">
          ...
          {engineOptions.map((e) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md transition-all font-medium ${selectedEngine === e ? (e.includes("vRS") || e.includes("GTI") ? "bg-red-600 text-white shadow-sm font-bold" : isVW ? "bg-blue-600 text-white shadow-sm" : "bg-emerald-600 text-white shadow-sm") : "bg-zinc-950 text-zinc-400 hover:text-white border border-zinc-800"}`}
            >
              {e}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {quickPrompts.map((prompt, idx) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 ${isVW ? "hover:border-blue-700/50" : "hover:border-emerald-700/50"} text-zinc-300 hover:text-white transition-all text-left cursor-pointer`}
            >
              {prompt}
            </button>
          ))}
      ...
      <div className="bg-zinc-900/60 rounded-2xl border border-zinc-800 p-4 sm:p-6 space-y-4 min-h-[360px] max-h-[580px] overflow-y-auto">
        {conversation.map((msg, index) => (
          ...
          >
            {msg.role === "assistant" && (
              <div
                className={`w-8 h-8 rounded-lg ${isVW ? "bg-blue-950 border border-blue-700/60 text-blue-400" : "bg-emerald-950 border border-emerald-700/60 text-emerald-400"} flex items-center justify-center flex-shrink-0`}
              >
            ...
            <div
              className={`max-w-[85%] rounded-2xl px-4 py-3 text-xs sm:text-sm leading-relaxed ${msg.role === "user" ? (isVW ? "bg-blue-600 text-white" : "bg-emerald-600 text-white") : "bg-zinc-950 border border-zinc-800 text-zinc-200 shadow-md"}`}
            >
                ...
                {msg.text.split("\n").map((line, lIdx) => {
                  if (line.startsWith("### ")) {
                    return (
                      <p
                        key={lIdx}
                        className={`font-bold ${isVW ? "text-blue-400" : "text-emerald-400"} text-base mt-2 mb-1`}
                      >
                        {line.replace("### ", "")}
                      </p>
                    );
      ...
      <div className="flex items-center gap-2 p-2 rounded-xl bg-zinc-900 border border-zinc-800">
        <input
          ...
          placeholder={`Ask about ${selectedModel === "All" ? (isVW ? "any Volkswagen model" : "any \u0160koda model") : selectedModel}, mileage, comparisons, or safety...`}
          className="flex-1 bg-transparent px-3 py-2 text-sm text-white placeholder-zinc-500 focus:outline-none"
        />
        <button
          onClick={() => handleAsk()}
          disabled={!question.trim() || loading}
          className={`px-4 py-2 rounded-lg ${isVW ? "bg-blue-600 hover:bg-blue-500 shadow-blue-900/40" : "bg-emerald-600 hover:bg-emerald-500 shadow-emerald-900/40"} disabled:opacity-40 text-white font-semibold text-xs flex items-center gap-1.5 transition-all shadow-md cursor-pointer`}
        >
          <span>Ask</span>
