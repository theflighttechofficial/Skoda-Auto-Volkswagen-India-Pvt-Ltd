...
import { MAN_SAFETY_SUITE, MAN_MODELS } from "../data/manData";
...
export const SafetyDeepDive = ({ brand = "skoda" }) => {
  ...
  const safetySuite = isMan ? MAN_SAFETY_SUITE : isScania ? SCANIA_SAFETY_SUITE : isSeat ? SEAT_SAFETY_SUITE : isBentley ? BENTLEY_SAFETY_SUITE : isLamborghini ? LAMBORGHINI_SAFETY_SUITE : isPorsche ? PORSCHE_SAFETY_SUITE : isAudi ? AUDI_SAFETY_SUITE : isVW ? VW_SAFETY_SUITE : SKODA_SAFETY_SUITE;
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div
            className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider mb-1 ${isMan ? "text-sky-400" : isScania ? "text-rose-400" : isSeat ? "text-orange-400" : isBentley ? "text-green-400" : isLamborghini ? "text-yellow-400" : isPorsche ? "text-amber-400" : isAudi ? "text-red-400" : isVW ? "text-blue-400" : "text-emerald-400"}`}
          >
            {isMan ? (
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
            {isMan
              ? "MAN AIS-113/AIS-052 Certified Safety Architecture"
              : isScania
          ...
          <p className="text-sm text-zinc-400">
            {isMan
              ? "Every current MAN truck and bus chassis sold in India is engineered to AIS-113/AIS-052 heavy-vehicle crash and cabin-integrity standards, backed by a reinforced ladder-frame chassis and driver-focused active safety systems"
              : isBentley
        ...
        <div
          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold self-start ${isMan ? "bg-sky-950/80 border-sky-800 text-sky-300" : isScania ? "bg-rose-950/80 border-rose-800 text-rose-300" : isSeat ? "bg-orange-950/80 border-orange-800 text-orange-300" : isBentley ? "bg-green-950/80 border-green-800 text-green-300" : isLamborghini ? "bg-yellow-950/80 border-yellow-800 text-yellow-300" : isPorsche ? "bg-amber-950/80 border-amber-800 text-amber-300" : isAudi ? "bg-red-950/80 border-red-800 text-red-300" : isVW ? "bg-blue-950/80 border-blue-800 text-blue-300" : "bg-emerald-950/80 border-emerald-800 text-emerald-300"}`}
        >
          <ShieldCheck
      ...
      <motion.div
        ...
        className={`relative overflow-hidden rounded-3xl bg-gradient-to-r via-zinc-900 to-zinc-950 border p-6 sm:p-8 shadow-xl ${isMan ? "from-sky-950/70 border-sky-800/40" : isScania ? "from-rose-950/70 border-rose-800/40" : isSeat ? "from-orange-950/70 border-orange-800/40" : isBentley ? "from-green-950/70 border-green-800/40" : isLamborghini ? "from-yellow-950/70 border-yellow-800/40" : isPorsche ? "from-amber-950/70 border-amber-800/40" : isAudi ? "from-red-950/70 border-red-800/40" : isVW ? "from-blue-950/70 border-blue-800/40" : "from-emerald-950/70 border-emerald-800/40"}`}
      >
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          <div className="lg:col-span-8 space-y-3">
            <div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${isMan ? "bg-sky-500/20 text-sky-300 border-sky-500/30" : isScania ? "bg-rose-500/20 text-rose-300 border-rose-500/30" : isSeat ? "bg-orange-500/20 text-orange-300 border-orange-500/30" : isBentley ? "bg-green-500/20 text-green-300 border-green-500/30" : isLamborghini ? "bg-yellow-500/20 text-yellow-300 border-yellow-500/30" : isPorsche ? "bg-amber-500/20 text-amber-300 border-amber-500/30" : isAudi ? "bg-red-500/20 text-red-300 border-red-500/30" : isVW ? "bg-blue-500/20 text-blue-300 border-blue-500/30" : "bg-emerald-500/20 text-emerald-300 border-emerald-500/30"}`}
            >
          ...
          <div className="lg:col-span-4 flex flex-col sm:flex-row lg:flex-col gap-3">
            <div className="p-4 rounded-2xl bg-zinc-950/90 border border-zinc-800 flex items-center justify-between shadow-sm">
              <div>
                ...
                <span
                  className={`text-xl font-bold ${isMan ? "text-sky-400" : isScania ? "text-rose-400" : isSeat ? "text-orange-400" : isBentley ? "text-green-400" : isLamborghini ? "text-yellow-400" : isPorsche ? "text-amber-400" : isAudi ? "text-red-400" : isVW ? "text-blue-400" : "text-emerald-400"}`}
                >
                  5-Star (29.71 / 34)
                </span>
              </div>
              <ShieldCheck
                className={`w-7 h-7 ${isMan ? "text-sky-400" : isScania ? "text-rose-400" : isSeat ? "text-orange-400" : isBentley ? "text-green-400" : isLamborghini ? "text-yellow-400" : isPorsche ? "text-amber-400" : isAudi ? "text-red-400" : isVW ? "text-blue-400" : "text-emerald-400"}`}
              />
            </div>
            <div className="p-4 rounded-2xl bg-zinc-950/90 border border-zinc-800 flex items-center justify-between shadow-sm">
              <div>
        ...
        {safetySuite.map((item, idx) => (
          ...
          >
            <div className="flex items-start justify-between flex-wrap gap-2">
              ...
              <span
                className={`px-2.5 py-1 rounded-lg text-xs font-bold whitespace-nowrap border ${isMan ? "bg-sky-950 border-sky-800 text-sky-300" : isScania ? "bg-rose-950 border-rose-800 text-rose-300" : isSeat ? "bg-orange-950 border-orange-800 text-orange-300" : isBentley ? "bg-green-950 border-green-800 text-green-300" : isLamborghini ? "bg-yellow-950 border-yellow-800 text-yellow-300" : isPorsche ? "bg-amber-950 border-amber-800 text-amber-300" : isAudi ? "bg-red-950 border-red-800 text-red-300" : isVW ? "bg-blue-950 border-blue-800 text-blue-300" : "bg-emerald-950 border-emerald-800 text-emerald-300"}`}
              >
                {item.metric}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h4 className="text-base font-bold text-white">
              ...
              {isMan ? "MAN" : isScania ? "Scania" : isSeat ? "SEAT" : isBentley ? "Bentley" : isLamborghini ? "Lamborghini" : isPorsche ? "Porsche" : isAudi ? "Audi" : isVW ? "Volkswagen" : "\u0160koda"} Portfolio
            </h4>
              ...
              {models.map((car) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`py-3 px-4 font-semibold ${isMan ? "text-sky-400" : isScania ? "text-rose-400" : isSeat ? "text-orange-400" : isBentley ? "text-green-400" : isLamborghini ? "text-yellow-400" : isPorsche ? "text-amber-400" : isAudi ? "text-red-400" : isVW ? "text-blue-400" : "text-emerald-400"}`}
                    >
                      {car.rating}
                    </td>
